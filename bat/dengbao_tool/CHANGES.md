...
=======
> - v2.10: 备案表默认值与云编号修正、报告局部标黄、对象构成/安全责任分段策略修正
> - v2.9: 定级报告标题字体统一、正文改仿宋_GB2312分段缩进、矩阵表仅交叉级别涂黑
...
=======
## v2.10 — 默认值、调查表云编号与报告局部标黄修正

### 文档生成修复

- **表二默认值兜底**：备案表中服务范围选“99-其他”且未填写说明时，生成端自动回填“本单位”；互联情况选“9-其他”且未填写说明时，自动回填“无连接”
- **云备案编号统一截断**：表四云计算“平台备案编号”继续按前两段截断，例如 `44032113609-00002-24-0162-01` 写为 `44032113609-00002`
- **安全责任保持单段**：定级报告中安全责任、责任主体、承载业务及业务/服务 1/2/3 小节改为单段写入，输入中的意外换行会自动并回正文，不再被拆成两段
- **对象构成保留分段**：定级对象构成继续按换行拆成多个自然段，并保持每段首行缩进
- **报告局部标黄**：定级报告标黄模式支持按文本框内选中的具体片段生成高亮，例如只高亮 `身份证号码：XXXXXXXXXXXXXXXXXX`，不再整段标黄

### 前端交互修复

- **服务范围/互联情况默认提示修正**：页面中“服务范围-其他”默认填“本单位”，“互联情况-其他”默认填“无连接”，不再统一误填“本单位”
- **调查表云编号覆盖预置值**：读取调查表后，云平台备案编号会覆盖预置服务商默认值，避免华为云等预置编号盖过调查表中的实际报告编号
- **互联网字段换行适配**：网络性质选择“互联网”时的源站 IP、域名、主要协议/端口输入框改为可换行布局，避免第三个输入框挤出界面
...
- `python -m py_compile app.py core\\doc_writer.py core\\doc_reader.py`
- 生成 `output/_verify_report_v30.docx`，验证安全责任为单段、对象构成按行分段、`身份证号码：XXXXXXXXXXXXXXXXXX` 仅该片段被标黄
- 生成 `output/_verify_beian_v30.docx`，验证服务范围“本单位”、互联情况“无连接”、云备案编号 `44032113609-00002` 已写入
...
- **标题字体统一**：定级报告首页标题中动态填入的系统名称改为强制使用 `方正小标宋简体` 二号，不再因占位 run 遗留 `Times New Roman`
- **正文样式统一**：定级报告正文替换后统一强制为 `FangSong_GB2312` 四号，并保留首行缩进两字符，不再出现替换后变成宋体四号的情况
- **多段正文真分段**：责任主体、定级对象构成、承载业务、安全责任以及业务/服务 1/2/3 小节支持按换行拆成多个自然段，每段分别首行缩进，解决“只第一段缩进”的问题
- **矩阵表涂黑收敛**：业务信息安全保护等级矩阵表、系统服务安全保护等级矩阵表改为只涂最终级别交叉单元格，不再连同行标题和列标题一起涂黑
...
- `python -m py_compile core\\doc_writer.py`
- 通过 `core.doc_writer.generate_report()` 生成验证文档，检查标题 run 东亚字体、正文段落字体/缩进，以及矩阵表仅交叉级别单元格存在底纹
>>>>>>> abbe1ee3b89a69f778cf4d76563b8d6501679cae

...
def cmd_check(args):
    ...
    with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), BarColumn(),
        ...
        task = progress.add_task("[cyan]检测中...", total=len(codex_files))
        with ThreadPoolExecutor(max_workers=args.concurrency) as pool:
            futures = {pool.submit(check_quota, f): f for f in codex_files}
            for future in as_completed(futures):
                results.append(future.result())
    ...
    console.print(Panel(f"[bold]检测 {len(results)} 个  发现 {len(invalid)} 个无效  已删除 {disabled_count} 个[/bold]",
                        border_style="red" if invalid else "green"))
...
def cmd_delete(args):
    ...
    console.print(
        Panel(f"[bold]共删除 {deleted_count} 个已禁用文件[/bold]", border_style="red" if deleted_count else "green"))
